...
function Profile() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const userTopArtistsLong = await getCurrentUserTopItems(
      'long_term',
      'artists',
      8
    );
    ...
    const userTopArtistsShort = await getCurrentUserTopItems(
      'short_term',
      'artists',
      8
    );
    ...
    const userTopTracksLong = await getCurrentUserTopItems(
      'long_term',
      'tracks',
      10
    );
    ...
    const userTopTracksShort = await getCurrentUserTopItems(
      'short_term',
      'tracks',
      10
    );
  ...
  const computedTrackTitle = `Top Tracks of ${
    useTracksLong === true ? 'All Time' : 'the Month'
  }`;
  const computedArtistsTitle = `Top Artists of ${
    useArtistsLong === true ? 'All Time' : 'the Month'
  }`;
          ...
          <div className="[ content__wrapper ]">
            {profile.images.length && profile.images[0].url && (
              <div className="[ profile__image ]">
                <img src={profile.images[0].url} alt="Spotify user profile" />
              </div>
            )}
